...
#' @param maps optionally, the output of \code{.getMaps()}
...
#' 
genDist <- function(graph, dims, maps, r=TRUE) {
  if (missing(maps)) maps <- .getMaps(graph, dims=dims, r=r)
  mobs <- mobs2 <- getMoebius(graph, makeptable(graph, dims=dims), r=r)
  ok <- FALSE
  gen <- function(x) {
  ...
  while(!ok) {
    ...
    if (all(getProbs(mobs2, maps, graph) >= 0)) ok = TRUE
  }
  mobs2
}
...
#' @param maps list containing maps for (use \code{.getMaps}).
...
function(moebius, maps, graph) {
  ...
  if (missing(maps)) maps = .getMaps(graph, dims=moebius$dims, r=moebius$r)
